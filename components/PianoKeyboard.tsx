...
export const PianoKeyboard: React.FC<PianoKeyboardProps> = ({ scaleNotes }) => {
    ...
    const normalizedScaleNotes = useMemo(() => new Set(scaleNotes.map(normalizeNote)), [scaleNotes]);
    
    const isNoteInScale = (keyName: string) => normalizedScaleNotes.has(normalizeNote(getNoteName(keyName)));
    const isRootNote = (keyName: string) => normalizeNote(getNoteName(keyName)) === normalizeNote(scaleNotes[0]);
