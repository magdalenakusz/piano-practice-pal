...
// --- Internal UI Components for PracticeScreen ---
...
const CompletionScreen: React.FC<CompletionScreenProps> = ({ onStartNewSession }) => (
    <div className="flex flex-col items-center justify-center text-center min-h-[28rem]">
        <svg className="w-24 h-24 text-green-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
...
const FeedbackButtons: React.FC<FeedbackButtonsProps> = ({ onFeedback }) => (
    <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <button onClick={() => onFeedback(1)} className="p-4 rounded-lg bg-red-600/80 hover:bg-red-600 transition-colors transform hover:scale-105">Needs Work</button>
            <button onClick={() => onFeedback(2)} className="p-4 rounded-lg bg-yellow-500/80 hover:bg-yellow-500 transition-colors transform hover:scale-105">Getting There</button>
            <button onClick={() => onFeedback(3)} className="p-4 rounded-lg bg-green-600/80 hover:bg-green-600 transition-colors transform hover:scale-105">Mastered</button>
        </div>
    </div>
);
    ...
    <div>
        ...
        <div className="mb-6">
            <div className="text-lg text-gray-300 mb-4 text-center sm:text-left p-2 rounded-md bg-gray-900/50">
                ...
                {equivalent && <p className="text-gray-400 text-base mt-1">Alt: <span className="font-mono tracking-tight">{equivalent.notes.join(' - ')}</span></p>}
            </div>

            <PianoKeyboard scaleNotes={scale.notes} />
            <div className="mt-4 flex items-center justify-center space-x-4">
                <div className="flex items-center space-x-2"><div className="w-4 h-4 rounded-full bg-green-500"></div><span className="text-sm">Root Note</span></div>
                <div className="flex items-center space-x-2"><div className="w-4 h-4 rounded-full bg-blue-500"></div><span className="text-sm">Scale Note</span></div>
            </div>
        </div>
    </div>
);
...
export const PracticeScreen: React.FC<UsePracticeDataReturn> = ({
    ...
    const onFeedback = (confidence: ConfidenceLevel) => {
        ...
        setPracticeStep('practice'); // Reset for the next scale
    };
    ...
    if (!currentScale) {
        // This case should ideally not happen if session is not complete, but it's a good fallback.
        return <div className="min-h-[28rem] flex items-center justify-center">No more scales for today.</div>;
    }
